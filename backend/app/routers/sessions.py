...
def current_session(db: DbSession = Depends(get_db)) -> Optional[Session]:
    return get_active_session(db)


@router.get("/last-interrupted", response_model=Optional[SessionDetail])
def last_interrupted_session(db: DbSession = Depends(get_db)) -> Optional[Session]:
...
def session_detail(session_id: int, db: DbSession = Depends(get_db)) -> Session:
...
def rebuild(db: DbSession = Depends(get_db)) -> RebuildResult:
    ...
    summary = rebuild_all_sessions(db)
...
def session_events(session_id: int, db: DbSession = Depends(get_db)) -> list[Event]:
...
def get_snapshot(session_id: int, db: DbSession = Depends(get_db)) -> SnapshotOut:
